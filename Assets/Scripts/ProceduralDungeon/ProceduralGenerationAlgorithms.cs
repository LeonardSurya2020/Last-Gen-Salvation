...
public static class ProceduralGenerationAlgorithms
{
    ...
    public static List<BoundsInt>BinarySpacePartitioning(BoundsInt spaceTopSplit, int minWidht, int minHeight)
    {
        Queue<BoundsInt> roomsQueue = new Queue<BoundsInt>();
        List<BoundsInt> roomsList = new List<BoundsInt>();
        roomsQueue.Enqueue(spaceTopSplit);
        ...
        while (roomsQueue.Count > 0)
        {
            ...
            if(room.size.y >= minHeight && room.size.x >= minWidht)
            {
                if(Random.value < 0.5f)
                {
                    ...
                    {
                        SplitVertically(minWidht, roomsQueue, room);
                    }
